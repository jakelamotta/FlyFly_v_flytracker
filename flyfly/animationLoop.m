...
for k=1:length(frameMatrix)
    ...
    while (n<=N)
        ...
        for z=1:(size(frameMatrix{k},1)-1)
            ...
            %Kristian change
            if frameMatrix{k}(end,n) == 0 && recording && flag
                fid_ = fopen('/home/motionvision/pipe','w');
                fwrite(fid_,'pause');
                fclose(fid_);
                recording = false;
            elseif frameMatrix{k}(end,n) ~= 0 && ~recording && flag
                fid_ = fopen('/home/motionvision/pipe','w');
                fwrite(fid_,'start');
                fclose(fid_);
                recording = true;
...
%Kristian change
%flag = false;
if ~getappdata(0,'network_on')
    toSend = struct;
    toSend.timeFinish = timeFinish;
    toSend.timeStart = timeStart;
    
    h = waitbar(0,'Please wait while parameter merging is performed');
    
    toSend.stim = Stimulus;
    if ~exist('debugData', 'var')
        toSend.debug = 'saving disabled';
    else
        debugData = rmfield(debugData,'dataLog');
        toSend.debug = debugData;
    end
    
    s1 = sendParam(toSend,10000);

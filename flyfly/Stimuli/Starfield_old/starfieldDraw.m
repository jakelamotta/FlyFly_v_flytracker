...
% 
% Parameters.D - Distance to monitor from the fly
% Parameters.dotSize - Size of dots when close to viewer
% Parameters.viewDist - How far away dots are visible
%
% Parameters.center  - fly position center (center of screen [x y] in px)
...
%OLOF COMMENT: rotations are converted from degrees to rad earlier, all
%this is correct
...
%NEW
ys(z>Parameters.drawDistance) = 6666;%ignore objects to far away
%END NEW
...
x(abs(x)> Parameters.constraints) = -(x(abs(x)> Parameters.constraints))*0.99; %*0.99 is to avoid blinking
y(abs(y)> Parameters.constraints) = -(y(abs(y)> Parameters.constraints))*0.99; %particles that jump back and forth 
z(abs(z)> Parameters.constraints) = -(z(abs(z)> Parameters.constraints))*0.99; %This is a quickfix that may not work under extreme conditions

if n == 5  %Olof asks: Why on trial 5? This should be on trial 1 if time is set really low(as in Starfield Impulse)
